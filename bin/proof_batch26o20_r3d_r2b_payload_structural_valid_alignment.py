...
BATCH = "26-O20-R3D-R2B"
PROOF_PATH = ROOT / "run/proofs/proof_batch26o20_r3d_r2b_payload_structural_valid_alignment.json"
MANIFEST_PATH = ROOT / "run/proofs/manifest_batch26o20_r3d_r2b_payload_structural_valid_alignment.json"
R2A_PATH = ROOT / "run/proofs/proof_batch26o20_r3d_r2a_structural_valid_source_field.json"
TARGET_FEATURES = ROOT / "app/mme_scalpx/services/features.py"
RUN_DIR = ROOT / "run/live_capture/batch26o20_r3d_r2b_payload_structural_valid_alignment"
...
TARGETS = [
    ...
    "run/proofs/proof_batch26o20_r3d_r2a_structural_valid_source_field.json",
]
...
def pgrep_service(service: str) -> list[str]:
    matches: list[str] = []
    self_name = "proof_batch26o20_r3d_r2b_payload_structural_valid_alignment.py"
...
def patch_features_return_payload_alignment(before: str) -> dict[str, Any]:
    marker = "Batch 26-O20-R3D-R2B returned payload structural_valid alignment"
    if marker in before:
        return {"patched": False, "already_present": True, "reason": "marker already present"}

    if "Batch 26-O20-R3D-R2A structural_valid source-field repair" not in before:
        return {"patched": False, "already_present": False, "reason": "R2A marker missing"}

    backup = ROOT / "run/_code_backups" / f"batch26o20_r3d_r2b_features_py_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pre_patch"
...
'''
    ...
    return {"patched": True, "already_present": False, "backup": str(backup), "reason": "FeatureService.run_once returned payload alignment appended"}
...
def run_feature_once(client: Any) -> dict[str, Any]:
    try:
        ...
        svc = F.FeatureService(
            ...
            instance_id="batch26o20-r3d-r2b-feature",
        )
...
def start_features_service() -> dict[str, Any]:
    ...
    log_path = RUN_DIR / "o20_r3d_r2b_features.log"
    pid_path = RUN_DIR / "o20_r3d_r2b_features.pid"
    args = [
        sys.executable,
        "-m",
        "app.mme_scalpx.main",
...
def run_strategy_once(client: Any) -> dict[str, Any]:
    try:
        ...
        svc = S.StrategyService(
            ...
            clock=type("Clock", (), {"now_ns": staticmethod(time.time_ns)})(),
            shutdown=type("S", (), {"is_set": staticmethod(lambda: True)})(),
            instance_id="batch26o20-r3d-r2b-strategy",
        )
        res = svc.run_once()
...
def main() -> int:
    result: dict[str, Any] = {
        "batch": BATCH,
        "batch_name": "payload_structural_valid_alignment",
        "created_at_utc": now_utc(),
        "scope": {
    ...
    r2a = load_json(R2A_PATH)
    req = r2a.get("required_verdicts", {}) if isinstance(r2a.get("required_verdicts"), Mapping) else {}
    false_keys = sorted([k for k, v in req.items() if v is not True])

    result["r2a_gate"] = {
        "exists": bool(r2a),
        "final_verdict": r2a.get("final_verdict"),
        "false_keys": false_keys,
        "required_verdicts": req,
    }

    if r2a.get("final_verdict") != "FAIL_O20_R3D_R2A_STRUCTURAL_VALID_SOURCE_FIELD_NOT_PROVEN":
        result["final_verdict"] = "FAIL_CLOSED_R2A_NOT_EXPECTED_FAILURE"
        write_outputs(result)
        return 2

    if false_keys != ["feature_once_payload_structural_valid"]:
        result["final_verdict"] = "FAIL_CLOSED_R2A_FALSE_KEYS_NOT_NARROW"
        result["next_recommended_batch"] = "Do not patch. Inspect R2A false keys."
        write_outputs(result)
        return 2
    ...
    patch_result = patch_features_return_payload_alignment(before_text)
    ...
    required = {
        ...
        "r2a_expected_fail_gate": r2a.get("final_verdict") == "FAIL_O20_R3D_R2A_STRUCTURAL_VALID_SOURCE_FIELD_NOT_PROVEN",
        "r2a_only_false_key_payload_structural_valid": false_keys == ["feature_once_payload_structural_valid"],
        ...
        "feature_once_payload_structural_valid_alias": feature_once.get("payload_consumer_view_structural_valid_alias") is True,
    ...
    if not all(required.values()):
        result["final_verdict"] = "FAIL_O20_R3D_R2B_PAYLOAD_STRUCTURAL_VALID_ALIGNMENT_NOT_PROVEN"
        result["next_recommended_batch"] = "Inspect R2B proof. Do not run O20-R3E."
        write_outputs(result)
        return 2

    result["final_verdict"] = "PASS_O20_R3D_R2B_PAYLOAD_STRUCTURAL_VALID_ALIGNMENT_OK_HOLD_ONLY"
