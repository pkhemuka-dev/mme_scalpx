...
BATCH = "26-O16H"
PROOF_PATH = ROOT / "run/proofs/proof_batch26o16h_final_data_valid_composition.json"
MANIFEST_PATH = ROOT / "run/proofs/manifest_batch26o16h_final_data_valid_composition.json"
O16G_R2_PATH = ROOT / "run/proofs/proof_batch26o16g_r2_runtime_symbol_provider_quality.json"
TARGET_FEATURES = ROOT / "app/mme_scalpx/services/features.py"
BACKUP_DIR = ROOT / "run/_code_backups" / f"batch26o16h_final_data_valid_composition_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def pgrep_service(service: str) -> list[str]:
    try:
        ...
        return []
    matches: list[str] = []
    self_name = "proof_batch26o16h_final_data_valid_composition.py"
...
def patch_features_final_composition(before_text: str) -> dict[str, Any]:
    marker = "Batch 26-O16H final selected-option common/provider-runtime data_valid composition bridge"
    if marker in before_text:
        return {"patched": False, "already_present": True, "reason": "O16H marker already present"}
    ...
    backup = BACKUP_DIR / "features.py.pre_o16h"
    shutil.copy2(TARGET_FEATURES, backup)

    patch = r'''

# =============================================================================
# Batch 26-O16H final selected-option common/provider-runtime data_valid composition bridge
# =============================================================================
#
# Safety:
...
# - Does not enable MISO; it preserves existing provider_ready_miso and
#   dhan_context_fresh truth exactly as produced upstream.
# - Composes feature frame validity only from existing futures/session/warmup
#   truth plus selected-option common quality and provider-runtime/classic truth.

def _batch26o16h_float(value: Any, default: float = 0.0) -> float:
    try:
...
def _batch26o16h_json(value: Any) -> dict[str, Any]:
    if not value:
        return {}
    if isinstance(value, bytes):
        value = value.decode("utf-8", "replace")
...
def _batch26o16h_decode_hash(raw: Mapping[Any, Any]) -> dict[str, str]:
    out: dict[str, str] = {}
    for k, v in dict(raw or {}).items():
        kk = k.decode("utf-8", "replace") if isinstance(k, bytes) else str(k)
        vv = v.decode("utf-8", "replace") if isinstance(v, bytes) else str(v)
...
def _batch26o16h_runtime_provider_truth(redis_obj: Any, family_features: Mapping[str, Any], selected: Mapping[str, Any]) -> dict[str, Any]:
    runtime = dict(family_features.get("provider_runtime", {}) or {})
    raw_runtime: dict[str, str] = {}
    try:
        raw_runtime = _batch26o16h_decode_hash(redis_obj.hgetall("state:provider_runtime") or {})
    except Exception:
        raw_runtime = {}

    for field in ("provider_runtime_json", "payload_json", "snapshot_json"):
        if field in raw_runtime:
            runtime.update(_batch26o16h_json(raw_runtime.get(field)))

    runtime.update({k: v for k, v in raw_runtime.items() if k not in runtime})

    selected_provider = str(
        selected.get("provider_id")
        or selected.get("provider")
        or runtime.get("active_selected_option_provider_id")
        or runtime.get("selected_option_provider")
        or runtime.get("active_execution_provider_id")
        or ""
    ).upper()

    futures_provider = str(
        runtime.get("active_futures_provider_id")
        or runtime.get("futures_provider")
        or ""
    ).upper()

    classic_provider_ready = bool(
        selected_provider in {getattr(N, "PROVIDER_ZERODHA", "ZERODHA"), getattr(N, "PROVIDER_DHAN", "DHAN")}
        or futures_provider in {getattr(N, "PROVIDER_ZERODHA", "ZERODHA"), getattr(N, "PROVIDER_DHAN", "DHAN")}
    )
    ...
    return {
        ...
        "classic_provider_ready": classic_provider_ready,
    }


def _batch26o16h_selected_quality(selected: Mapping[str, Any]) -> dict[str, Any]:
    side = str(selected.get("side") or selected.get("option_side") or "").upper()
    ltp = _batch26o16h_float(selected.get("ltp") or selected.get("last_price"), 0.0)
    spread = _batch26o16h_float(selected.get("spread"), 0.0)
    spread_ratio = _batch26o16h_float(selected.get("spread_ratio"), 0.0)
    depth_total = int(_batch26o16h_float(selected.get("depth_total"), 0.0))
    best_bid = _batch26o16h_float(selected.get("best_bid"), 0.0)
    best_ask = _batch26o16h_float(selected.get("best_ask"), 0.0)
    ...
    # If best bid/ask are not carried on selected common but spread/ratio/depth
    # are carried, do not fail quality solely because bid/ask keys are absent.
...
if "_BATCH26O16H_ORIGINAL_FEATURESERVICE_RUN_ONCE" not in globals() and "FeatureService" in globals():
    _BATCH26O16H_ORIGINAL_FEATURESERVICE_RUN_ONCE = FeatureService.run_once

    def _batch26o16h_run_once(self: FeatureService, *args: Any, **kwargs: Any) -> dict[str, Any]:
        payload = dict(_BATCH26O16H_ORIGINAL_FEATURESERVICE_RUN_ONCE(self, *args, **kwargs))
        ...
        quality = _batch26o16h_selected_quality(selected)
        provider_truth = _batch26o16h_runtime_provider_truth(self.redis, family_features, selected)
        ...
        if provider_truth["classic_provider_ready"]:
            flags["provider_ready_classic"] = True

        # Classic data quality requires real selected option quality + classic provider.
        ...
        # Preserve MISO/Dhan truth exactly.
        ...
        selected["source_bridge"] = "batch26o16h"
        if provider_truth["selected_provider"] and not selected.get("provider_id"):
            selected["provider_id"] = provider_truth["selected_provider"]
        ...
        if family_surfaces:
            ...
            hash_payload = {
                ...
                "o16h_composition_json": _json_dump({
    ...
    FeatureService.run_once = _batch26o16h_run_once
'''
    TARGET_FEATURES.write_text(before_text.rstrip() + "\n" + patch + "\n", encoding="utf-8")
    return {"patched": True, "already_present": False, "backup": str(backup), "reason": "O16H final composition bridge appended"}
...
def main() -> int:
    result: dict[str, Any] = {
        "batch": BATCH,
        "batch_name": "final_data_valid_composition",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "scope": {
            "guarded": True,
            "paper_restart": False,
            ...
            "allowed_patch": "features.py selected-option common + provider-runtime data_valid composition only",
        },
    }
    ...
    o16g_r2 = safe_json_load(O16G_R2_PATH.read_text(encoding="utf-8")) if O16G_R2_PATH.exists() else {}
    result["o16g_r2_gate"] = {
        "exists": O16G_R2_PATH.exists(),
        "final_verdict": o16g_r2.get("final_verdict") if isinstance(o16g_r2, Mapping) else None,
        "required_verdicts": o16g_r2.get("required_verdicts") if isinstance(o16g_r2, Mapping) else None,
    }

    if o16g_r2.get("final_verdict") != "FAIL_O16G_R2_PROVIDER_DATA_QUALITY_REPAIR_NOT_PROVEN":
        result["final_verdict"] = "FAIL_CLOSED_O16G_R2_NOT_EXPECTED_VERDICT"
        result["patch_performed"] = False
        write_outputs(result)
        return 2
    ...
    if patch_allowed:
        patch_result = patch_features_final_composition(before_text)
    else:
        patch_result = {"patched": False, "already_present": False, "reason": "O16H patch gate not satisfied"}
    ...
    post_script = ROOT / "run/proofs/_tmp_batch26o16h_post_check.py"
...
composition = json.loads(raw.get("o16h_composition_json") or "{}")
...
out = {
    ...
    "o16h_composition_present": bool(composition),
    "o16h_composition": composition,
}
...
''', encoding="utf-8")
    ...
    required = {
        ...
        "o16g_r2_expected_failure_gate": result["o16g_r2_gate"]["final_verdict"] == "FAIL_O16G_R2_PROVIDER_DATA_QUALITY_REPAIR_NOT_PROVEN",
    ...
    if all([
        ...
        result["final_verdict"] = "PASS_O16H_RUNTIME_DATA_VALID_SAFE_TO_CONSUME_OK"
        result["next_recommended_batch"] = "26-O17 activation candidate extraction proof, no risk/execution"
        write_outputs(result)
        return 0

    result["final_verdict"] = "FAIL_O16H_FINAL_DATA_VALID_COMPOSITION_NOT_PROVEN"
