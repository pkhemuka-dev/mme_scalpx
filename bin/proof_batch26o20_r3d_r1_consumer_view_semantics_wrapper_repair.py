...
BATCH = "26-O20-R3D-R1"
PROOF_PATH = ROOT / "run/proofs/proof_batch26o20_r3d_r1_consumer_view_semantics_wrapper_repair.json"
MANIFEST_PATH = ROOT / "run/proofs/manifest_batch26o20_r3d_r1_consumer_view_semantics_wrapper_repair.json"
O20R3D_PATH = ROOT / "run/proofs/proof_batch26o20_r3d_consumer_view_validity_semantics.json"
TARGET_FEATURES = ROOT / "app/mme_scalpx/services/features.py"
RUN_DIR = ROOT / os.environ.get("BATCH26O20_R3D_R1_RUN_DIR", "run/live_capture/batch26o20_r3d_r1_features_only")
...
TARGETS = [
    ...
    "run/proofs/proof_batch26o20_r3d_consumer_view_validity_semantics.json",
]
...
def pgrep_service(service: str) -> list[str]:
    matches: list[str] = []
    self_name = "proof_batch26o20_r3d_r1_consumer_view_semantics_wrapper_repair.py"
...
def patch_features_semantics_direct_wrapper(before: str) -> dict[str, Any]:
    marker = "Batch 26-O20-R3D-R1 direct consumer-view semantics wrapper"
    if marker in before:
        return {"patched": False, "already_present": True, "reason": "marker already present"}

    if "Batch 26-O20-R3D consumer-view validity semantics guard" not in before:
        return {"patched": False, "already_present": False, "reason": "O20-R3D marker missing"}

    # R3D failed because the second __init__ wrapper wrapped the first wrapper's
    # already-wrapped Redis object; R3A's guard could bypass R3D's hset branch.
    # R1 replaces the fragile stacked wrapper with an unconditional module-level
    # direct patch of FeatureService.run_once: run original, then repair the
    # actual Redis HASH_FEATURES values already written, and also repair returned
    # payload when possible.
    backup = ROOT / "run/_code_backups" / f"batch26o20_r3d_r1_features_py_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pre_patch"
...
'''
    ...
    return {"patched": True, "already_present": False, "backup": str(backup), "reason": "direct FeatureService.run_once semantics wrapper appended"}
...
def feature_snapshot(client: Any, features_hash_key: str) -> dict[str, Any]:
    ...
    sem_r3d = hget_json(client, features_hash_key, "o20r3d_validity_semantics_json")
    sem_r1 = hget_json(client, features_hash_key, "o20r3d_r1_validity_semantics_json")
    ...
    return {
        ...
        "semantics_guard_present_r3d": bool(sem_r3d),
        "semantics_guard_present_r1": bool(sem_r1),
        "semantics_guard_r1": sem_r1,
    }
...
def run_feature_once(client: Any) -> dict[str, Any]:
    try:
        ...
        svc = F.FeatureService(
            ...
            instance_id="batch26o20-r3d-r1-feature-semantics",
        )
...
def run_strategy_once(client: Any) -> dict[str, Any]:
    try:
        ...
        svc = S.StrategyService(
            ...
            clock=type("Clock", (), {"now_ns": staticmethod(time.time_ns)})(),
            shutdown=type("S", (), {"is_set": staticmethod(lambda: True)})(),
            instance_id="batch26o20-r3d-r1-strategy-semantics",
        )
        res = svc.run_once()
...
def start_features_service() -> dict[str, Any]:
    ...
    log_path = RUN_DIR / "o20_r3d_r1_features.log"
    pid_path = RUN_DIR / "o20_r3d_r1_features.pid"
    args = [
        sys.executable,
        "-m",
        "app.mme_scalpx.main",
...
def main() -> int:
    result: dict[str, Any] = {
        "batch": BATCH,
        "batch_name": "consumer_view_semantics_wrapper_repair",
        "created_at_utc": now_utc(),
        "scope": {
    ...
    o20r3d = load_json(O20R3D_PATH)
    result["o20r3d_gate"] = {
        "exists": bool(o20r3d),
        "final_verdict": o20r3d.get("final_verdict"),
        "required_verdicts": o20r3d.get("required_verdicts"),
    }

    if o20r3d.get("final_verdict") != "FAIL_O20_R3D_CONSUMER_VIEW_VALIDITY_SEMANTICS_NOT_PROVEN":
        result["final_verdict"] = "FAIL_CLOSED_O20R3D_NOT_EXPECTED_FAILURE"
    ...
    patch_result = patch_features_semantics_direct_wrapper(before_text)
    ...
    required = {
        ...
        "o20r3d_expected_fail_gate": o20r3d.get("final_verdict") == "FAIL_O20_R3D_CONSUMER_VIEW_VALIDITY_SEMANTICS_NOT_PROVEN",
        ...
        "snapshot_after_once_payload_cv_safe_true": snapshot_after_once.get("payload_consumer_view_safe_to_consume") is True,
        ...
        "r1_semantics_guard_present": snapshot_after_once.get("semantics_guard_present_r1") is True,
    ...
    if not all(required.values()):
        result["final_verdict"] = "FAIL_O20_R3D_R1_CONSUMER_VIEW_SEMANTICS_WRAPPER_REPAIR_NOT_PROVEN"
        result["next_recommended_batch"] = "Inspect O20-R3D-R1 proof/logs. Do not rerun O20-R3E."
        write_outputs(result)
        return 2

    result["final_verdict"] = "PASS_O20_R3D_R1_CONSUMER_VIEW_SEMANTICS_WRAPPER_REPAIR_OK_HOLD_ONLY"
