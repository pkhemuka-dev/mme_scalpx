...
"""
Offline proof for services/strategy_family/misb.py.

This proof:
- builds deterministic StrategyFamilyConsumerView-like mappings
- proves MISB CALL candidate
- proves MISB PUT candidate
- proves no-signal when breakout support is weak
...
from app.mme_scalpx.services.strategy_family import misb
    ...
    return {
        ...
        "common": {
            ...
            "futures": {
                ...
                "depth_total": 2400.0,
                "depth_ok": True,
                "ofi_ratio_proxy": 0.30 if selected_side == SIDE_CALL else -0.30,
                "nof_slope": 0.10 if selected_side == SIDE_CALL else -0.10,
                "delta_3": 5.0 if selected_side == SIDE_CALL else -5.0,
                "velocity_ratio": 1.80,
                "vol_norm": 1.50,
        ...
        "family_surfaces": {
            "surfaces_by_branch": {
                "misb_call": {
                    ...
                    "side": SIDE_CALL,
                    "eligible": True,
                    "futures_bias_ok": selected_side == SIDE_CALL,
                    "shelf_valid": True,
                ...
                "misb_put": {
                    ...
                    "side": SIDE_PUT,
                    "eligible": True,
                    "futures_bias_ok": selected_side == SIDE_PUT,
                    "shelf_valid": True,
                    "breakout_triggered": True,
                    "breakout_accepted": True,
                    "context_pass": True,
...
def main() -> int:
    ...
    call_result = misb.evaluate(call_view, branch_id=BRANCH_CALL)
    assert call_result.is_candidate, call_result
    assert call_result.candidate is not None
    assert call_result.candidate.family_id == "MISB"
    ...
    put_result = misb.evaluate(put_view, branch_id=BRANCH_PUT)
    ...
    weak_surface = weak_view["family_surfaces"]["surfaces_by_branch"]["misb_call"]
    weak_surface["shelf_valid"] = False
    weak_surface["breakout_triggered"] = False
    weak_surface["breakout_accepted"] = False
    weak_view["common"]["futures"]["velocity_ratio"] = 0.75
    weak_view["common"]["call"]["response_efficiency"] = 0.01
    weak_result = misb.evaluate(weak_view, branch_id=BRANCH_CALL)
    ...
    blocked = misb.evaluate(degraded_view, branch_id=BRANCH_CALL)
    ...
    print("MISB doctrine offline proof: OK")
    return 0
