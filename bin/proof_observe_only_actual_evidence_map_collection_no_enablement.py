...
SCAN_FILES = [
    Path("app/mme_scalpx/replay/live_package_collector.py"),
    Path("bin/observe_only_market_session_package_collect.py"),
]
...
def main() -> int:
    ...
    parser.add_argument("--out", default="run/proofs/proof_observe_only_actual_evidence_map_collection_no_enablement.json")
    args = parser.parse_args()

    collection_contract = load_json(Path("run/proofs/proof_observe_only_actual_evidence_map_collection_contract.json"))
    ...
    ok = bool(
        static_scan_ok
        and collection_contract.get("observe_only_actual_evidence_map_collection_contract_ok") is True
        and collection_contract.get("starts_services") is False
        and collection_contract.get("reads_live_redis") is False
        and collection_contract.get("writes_live_redis") is False
        and collection_contract.get("calls_broker_api") is False
        and collection_contract.get("paper_armed_approved") is False
        and collection_contract.get("live_trading_approved") is False
        and collection_contract.get("full_live_replay_parity") == "NOT_PROVEN_IN_28F"
    ...
    proof = {
        "schema_version": "proof_observe_only_actual_evidence_map_collection_no_enablement_v1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "observe_only_actual_evidence_map_collection_no_enablement_ok": ok,
        ...
        "verdict": "PASS_OBSERVE_ONLY_ACTUAL_EVIDENCE_MAP_COLLECTION_NO_ENABLEMENT" if ok else "FAIL_REVIEW_REQUIRED",
    }
