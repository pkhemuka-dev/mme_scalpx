...
CHILD_EXPECTED_EXECUTION_SHA="94744378b4b7a7ae2d4754a0f1dbf87a06861b0dd8973639b7beece44abcd0a2"
...
# R38VXFFDP current one-event runner successor guard.
R38VXFFDP_CURRENT_POLICY_ACK="R38TK3_FAMILY_CONTRACT_EXIT_V3_TARGET5_STOP4_STRUCTURAL_TIME300"

if [ "${1:-}" = "--r38vxffdp-current-runner-selftest" ]; then
    echo "SELFTEST_CLASSIFICATION=PASS_R38VXFFDP_CURRENT_ONE_EVENT_RUNNER_SUCCESSOR"
    echo "RUNNER_SUCCESSOR_DEFAULT_ENABLED=0"
    echo "CURRENT_POLICY_ACK=$R38VXFFDP_CURRENT_POLICY_ACK"
    echo "EXPECTED_STRATEGY_SHA=b6374975d85f7bfe4c44dbe1188caef3fb4fdd316e4323a513b25329a9d5fc52"
    echo "EXPECTED_EXECUTION_SHA=94744378b4b7a7ae2d4754a0f1dbf87a06861b0dd8973639b7beece44abcd0a2"
    echo "EXPECTED_MANAGER_SHA=ca2742bf5d6b1fd127b7cab7b219ebe1639a626de1c8c288a8e99b0e4e8cf5a0"
    echo "BASE_RUNNER_CHANGED=0"
    echo "PAPER_STARTED=0"
    echo "LIVE_STARTED=0"
    exit 0
fi

if [ "${R38VXFFDP_ENABLE_CURRENT_RUNNER_SUCCESSOR:-0}" != "1" ]; then
    echo "FINAL_CLASSIFICATION=BLOCK_CURRENT_ONE_EVENT_RUNNER_SUCCESSOR_DISABLED_BY_DEFAULT"
    echo "REASON=REQUIRES_EXPLICIT_FRIDAY_CONTROLLED_PAPER_AUTHORIZATION_PACKAGE"
    echo "CURRENT_POLICY_ACK=$R38VXFFDP_CURRENT_POLICY_ACK"
    echo "PAPER_STARTED=0"
    echo "LIVE_STARTED=0"
    exit 1
...
EXIT_POLICY_ACK="R38TK3_FAMILY_CONTRACT_EXIT_V3_TARGET5_STOP4_STRUCTURAL_TIME300"
...
broker_readonly_flat() {
    local stage="$1"
    local report="$ROOT/outputs/broker_${stage}.json"
    local log="$ROOT/outputs/broker_${stage}.txt"
...
cleanup_on_error() {
    ...
    if ! broker_readonly_flat "failure_cleanup"; then
        echo "CRITICAL_BROKER_FLAT_NOT_PROVEN=1"
        echo "FAIL_SAFE_ACTION=KEEP_EXACT_PAPER_STRATEGY_RISK_EXECUTION_RUNNING"
        echo "DO_NOT_KILL_SERVICES_WHEN_BROKER_STATE_UNPROVEN=1"
        exit "$rc"
    fi
...
broker_readonly_flat "initial" || {
    echo "STOP_INITIAL_BROKER_NOT_PROVEN_FLAT_OR_ACTIVE_ORDER_PRESENT"
    exit 96
}

echo "PASS_INITIAL_LOCAL_AND_BROKER_FLAT_SAFETY"
...
BROKER_GATE_SHA="$(sha256sum "$BROKER_GATE" | awk '{print $1}')"
echo "broker_gate_sha=$BROKER_GATE_SHA"
[ "$BROKER_GATE_SHA" = "$EXPECTED_BROKER_GATE_SHA" ] || {
    echo "STOP_BROKER_READONLY_GATE_SHA_DRIFT"
    exit 95
}

echo "PASS_EXIT_MANAGER_RESOLVER_AND_BROKER_GATE_SOURCE_IDENTITY_GUARD"
...
grep -q \
  "R38TK3_FAMILY_CONTRACT_EXIT_V3" \
  "$MANAGER_SRC"
...
broker_readonly_flat "pre_stop" || {
    echo "CRITICAL_BROKER_FLAT_NOT_PROVEN_AFTER_LOCAL_EXIT=1"
...
broker_readonly_flat "final" || {
    echo "CRITICAL_FINAL_BROKER_FLAT_NOT_PROVEN=1"
    echo "DO_NOT_KILL_OBSERVE_SERVICES=1"
    exit 98
}

echo "PASS_OBSERVE_ONLY_RESTORED_AND_BROKER_FLAT"
...
LOCAL_FLAT=1
BROKER_FLAT=1
BROKER_NONFLAT_POSITION_COUNT=0
ACTIVE_ORDER_COUNT=0
LEDGER_JSON=$ROOT/outputs/lifecycle_audit.json
STRUCTURAL_EXIT_ENABLED=0
NEXT=REVIEW_R38VBBASE_PAIRED_PNL_AND_REPEAT_NATURAL_PAPER_EVENTS
EOF

cp -a "$ROOT/SUMMARY.txt" "$ROOT/summary.env"
...
echo "SUMMARY_ENV=$ROOT/summary.env"
echo "LEDGER_JSON=$ROOT/outputs/lifecycle_audit.json"
echo "BUNDLE=$ARCHIVE"
