...
def import_candidate(project_root: pathlib.Path, module_file: str, candidate_name: str) -> dict[str, Any]:
    ...
    try:
        spec = importlib.util.spec_from_file_location("replay_engine_hook_28z_direct", str(module_path))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Batch 28Z guarded ReplayEngineHook execution.")
    ...
    manifest = {
        "schema_version": "replay_engine_hook_guarded_execution_manifest_28z_v1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "accepted_for": "GUARDED_REPLAY_ENGINE_HOOK_EXECUTION_ONLY",
        "selected_candidate": {
    ...
    write_json(output_root / "03_no_broker_no_live_redis_boundary.json", {
        "schema_version": "no_broker_no_live_redis_boundary_28z_v1",
        "starts_services": False,
        "reads_live_redis": False,
        "writes_live_redis": False,
        "calls_broker_api": False,
    ...
    write_json(output_root / "04_next_replay_output_inspection_contract.json", {
        "schema_version": "next_replay_output_inspection_contract_28z_v1",
        "execution_ok": bool(execution_ok),
    ...
    result = {
        "schema_version": "replay_engine_hook_guarded_execution_result_28z_v1",
        "accepted_for": "GUARDED_REPLAY_ENGINE_HOOK_EXECUTION_ONLY",
        "execution_ok": bool(execution_ok),
        "execution_attempted": bool(execution_attempted),
        "instance_created": bool(instance_created),
