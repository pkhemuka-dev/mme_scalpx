...
BATCH = "26-O20-R3B"
RUN_SECONDS = int(os.environ.get("BATCH26O20_R3B_RUN_SECONDS", "180"))
RUN_SECONDS = max(120, min(RUN_SECONDS, 300))
SAMPLE_COUNT = int(os.environ.get("BATCH26O20_R3B_SAMPLE_COUNT", "8"))
SAMPLE_COUNT = max(6, min(SAMPLE_COUNT, 12))

PROOF_PATH = ROOT / "run/proofs/proof_batch26o20_r3b_corrected_bounded_observation.json"
MANIFEST_PATH = ROOT / "run/proofs/manifest_batch26o20_r3b_corrected_bounded_observation.json"
O20R3A_PATH = ROOT / "run/proofs/proof_batch26o20_r3a_persistent_features_abi_publish_repair.json"
RUN_DIR = ROOT / os.environ.get("BATCH26O20_R3B_RUN_DIR", "run/live_capture/batch26o20_r3b_corrected_bounded")
...
def pgrep_service(service: str) -> list[str]:
    matches: list[str] = []
    self_name = "proof_batch26o20_r3b_corrected_bounded_observation.py"
...
def start_service(service: str) -> dict[str, Any]:
    ...
    log_path = RUN_DIR / f"o20_r3b_{service}.log"
    pid_path = RUN_DIR / f"o20_r3b_{service}.pid"
    args = [
        sys.executable,
        "-m",
        "app.mme_scalpx.main",
...
def run_feature_once_hygiene(client: Any) -> dict[str, Any]:
    try:
        ...
        svc = F.FeatureService(
            ...
            instance_id="batch26o20-r3b-feature-hygiene",
        )
...
def run_strategy_one_shot_hygiene(client: Any) -> dict[str, Any]:
    try:
        ...
        svc = S.StrategyService(
            ...
            clock=type("Clock", (), {"now_ns": staticmethod(time.time_ns)})(),
            shutdown=type("S", (), {"is_set": staticmethod(lambda: True)})(),
            instance_id="batch26o20-r3b-strategy-hygiene",
        )
        res = svc.run_once()
...
def write_fail_safe(reason: str) -> None:
    try:
        ...
        write_outputs({
            "batch": BATCH,
            "batch_name": "corrected_bounded_observation_after_persistent_abi_repair",
            "created_at_utc": now_utc(),
...
def main() -> int:
    result: dict[str, Any] = {
        ...
        "scope": {
            ...
            "nohup_safe": True,
            "after_persistent_features_abi_repair": True,
        },
    }
    ...
    o20r3a = load_json(O20R3A_PATH)
    result["o20r3a_gate"] = {
        "exists": bool(o20r3a),
        "final_verdict": o20r3a.get("final_verdict"),
        "required_verdicts": o20r3a.get("required_verdicts"),
        "next_recommended_batch": o20r3a.get("next_recommended_batch"),
    }

    if o20r3a.get("final_verdict") != "PASS_O20_R3A_PERSISTENT_FEATURES_ABI_PUBLISH_REPAIR_OK":
        result["final_verdict"] = "FAIL_CLOSED_O20R3A_NOT_PASS"
        result["next_recommended_batch"] = "Do not rerun bounded observation. Inspect O20-R3A."
        write_outputs(result)
        return 2
    ...
    feature_hygiene = run_feature_once_hygiene(client)
        ...
        for service in start_order:
            ...
            if service == "features":
                ...
                if not (
                    ...
                    and mid_feature.get("branch_frame_count") == 10
                ):
                ...
                time.sleep(10)
                recent_errors = latest_rows(client, errors_key, count=30)
    ...
    log_paths = []
    for service in start_order:
        p = RUN_DIR / f"o20_r3b_{service}.log"
        if p.exists():
            log_paths.append(p)
    ...
    samples_rich_absent = sum(1 for s in samples if s.get("feature_snapshot", {}).get("selected_option_rich_in_common") is False)
    ...
    required = {
        "o20r3a_pass_gate": result["o20r3a_gate"]["final_verdict"] == "PASS_O20_R3A_PERSISTENT_FEATURES_ABI_PUBLISH_REPAIR_OK",
        ...
        "selected_option_rich_absent_in_samples": samples_rich_absent >= max(1, SAMPLE_COUNT - 1),
        ...
        "features_stopped_after_run": len(post_processes["features"]) == 0,
        "strategy_stopped_after_run": len(post_processes["strategy"]) == 0,
        "risk_stopped_after_run": len(post_processes["risk"]) == 0,
        "execution_stopped_after_run": len(post_processes["execution"]) == 0,
    ...
    if not all(required.values()):
        result["final_verdict"] = "FAIL_O20_R3B_CORRECTED_BOUNDED_OBSERVATION_NOT_PROVEN"
        result["next_recommended_batch"] = "Inspect O20-R3B proof/logs. Do not proceed to O22-R2 or O23."
        write_outputs(result)
        return 2

    result["final_verdict"] = "PASS_O20_R3B_CORRECTED_BOUNDED_OBSERVATION_OK_NO_ORDER"
    result["next_recommended_batch"] = "26-O22-R2 longer observation plan proof correction using O20-R3B; still no real live"
    write_outputs(result)
    return 0
