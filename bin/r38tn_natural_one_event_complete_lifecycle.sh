...
TAG="LANE-X-R38TN_NATURAL_ONE_EVENT_ENTRY_STRATEGY_EXIT_FLAT_$(date +%Y%m%d_%H%M%S)"
ROOT="run/proofs/$TAG"
...
EXPECTED_STRATEGY_SHA="a40a7070f1d2feb47e90c4ce9faf529011ad9e9487fd6895ebbfd039f625cdba"
...
echo "===== $TAG ====="
date -Is
echo "CURRENT_LOCATION=LANE_X_R38TN"
...
classification = (
    "PASS_R38TN_ROLLING_EXACT_QUOTE_GATE"
    if pair_pass >= 5
    else
    "BLOCK_R38TN_ROLLING_EXACT_QUOTE_GATE"
)
...
grep -q \
  "CLASSIFICATION=PASS_R38TN_ROLLING_EXACT_QUOTE_GATE" \
...
locked.update(
    {
        "classification":
            "PASS_R38TN_REAL_CANDIDATE_LOCKED",
        "generated_ack":
            expected_ack,
        "candidate_age_ms":
            age_ms,
...
[ "$ENTRY_SEEN" = "1" ] || {
    echo "CLASSIFICATION=REVIEW_R38TN_NO_NATURAL_ENTRY_WITHIN_TIMEOUT_FLAT"
    exit 45
}
...
[ "$EXIT_SEEN" = "1" ] || {
    echo "BLOCK_R38TN_NATURAL_EXIT_TIMEOUT_POSITION_REMAINS_OPEN"
...
# BEGIN R38TN_ORDER_STREAM_SEMANTICS_V2
#
...
# END R38TN_ORDER_STREAM_SEMANTICS_V2

classification = (
    "PASS_R38TN_NATURAL_ONE_EVENT_COMPLETE_STRATEGY_EXIT_FLAT"
    if passed
    else
    "BLOCK_R38TN_LIFECYCLE_EVIDENCE_MISMATCH"
)
...
grep -q \
  "CLASSIFICATION=PASS_R38TN_NATURAL_ONE_EVENT_COMPLETE_STRATEGY_EXIT_FLAT" \
...
FINAL_POSITION=FLAT
NEXT=REVIEW_R38TN_PAIRED_PNL_AND_REPEAT_NATURAL_PAPER_EVENTS
EOF
...
echo
echo "===== PASS ====="
echo "CLASSIFICATION=PASS_R38TN_NATURAL_ONE_EVENT_COMPLETE_STRATEGY_EXIT_FLAT"
