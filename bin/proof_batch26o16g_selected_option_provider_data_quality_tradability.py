...
BATCH = "26-O16G"
PROOF_PATH = ROOT / "run/proofs/proof_batch26o16g_selected_option_provider_data_quality_tradability.json"
MANIFEST_PATH = ROOT / "run/proofs/manifest_batch26o16g_selected_option_provider_data_quality_tradability.json"
O16F_PATH = ROOT / "run/proofs/proof_batch26o16f_selected_option_source_to_feature_mapping.json"
TARGET_FEATURES = ROOT / "app/mme_scalpx/services/features.py"
BACKUP_DIR = ROOT / "run/_code_backups" / f"batch26o16g_selected_option_provider_data_quality_tradability_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
TARGETS = [
    ...
    "run/proofs/proof_batch26o16f_selected_option_source_to_feature_mapping.json",
]
...
def pgrep_service(service: str) -> list[str]:
    try:
        ...
        return []
    matches: list[str] = []
    self_name = "proof_batch26o16g_selected_option_provider_data_quality_tradability.py"
...
def patch_features_provider_quality_bridge(before_text: str) -> dict[str, Any]:
    marker = "Batch 26-O16G selected-option provider/data-quality/tradability bridge"
    if marker in before_text:
        return {"patched": False, "already_present": True, "reason": "O16G marker already present"}
    ...
    backup = BACKUP_DIR / "features.py.pre_o16g"
    shutil.copy2(TARGET_FEATURES, backup)

    patch = r'''

# =============================================================================
# Batch 26-O16G selected-option provider/data-quality/tradability bridge
# =============================================================================
#
# Safety:
...
# - Does not force data_valid without existing futures + selected option +
#   classic provider + quote quality + session/warmup truth.
# - Repairs only the already-proven selected-option provider/quality mapping.

def _batch26o16g_float(value: Any, default: float = 0.0) -> float:
    try:
...
def _batch26o16g_decode_hash(raw: Mapping[Any, Any]) -> dict[str, str]:
    out: dict[str, str] = {}
    for k, v in dict(raw or {}).items():
        kk = k.decode("utf-8", "replace") if isinstance(k, bytes) else str(k)
        vv = v.decode("utf-8", "replace") if isinstance(v, bytes) else str(v)
...
def _batch26o16g_latest_stream(redis_obj: Any, key: str) -> dict[str, Any]:
    try:
        rows = redis_obj.xrevrange(key, count=1)
        if not rows:
            return {}
        msg_id, fields = rows[0]
        out = _batch26o16g_decode_hash(fields)
        out["_stream_key"] = key
...
def _batch26o16g_source_ok(raw: Mapping[str, Any]) -> bool:
    if not isinstance(raw, Mapping) or not raw:
        return False
    provider_role = str(raw.get("provider_role") or "").lower()
    stream_key = str(raw.get("_stream_key") or "").lower()
    ...
    instrument_key = raw.get("instrument_key") or raw.get("trading_symbol") or raw.get("tradingsymbol")
    has_core = bool(provider_id and instrument_key and ("CALL" in side or "PUT" in side or "CE" in side or "PE" in side))
    selected_lane = bool("selected" in stream_key or "selected_option" in provider_role)
    return bool(has_core and selected_lane)


def _batch26o16g_best_selected_source(redis_obj: Any, selected_side: str = "") -> dict[str, Any]:
    candidates = []
    for key in (
        ...
        item = _batch26o16g_latest_stream(redis_obj, key)
        if _batch26o16g_source_ok(item):
            candidates.append(item)

    if not candidates:
        return {}

    def score(raw: Mapping[str, Any]) -> tuple[int, int, float]:
        provider_id = str(raw.get("provider_id") or "").upper()
        tick_validity = str(raw.get("tick_validity") or "").upper()
        reject_reason = str(raw.get("reject_reason") or "")
        side = str(raw.get("option_side") or raw.get("instrument_role") or "").upper()
        side_ok = bool(selected_side and selected_side.upper() in side)
        clean = bool(tick_validity == "OK" and not reject_reason)
        provider_score = 2 if provider_id == getattr(N, "PROVIDER_ZERODHA", "ZERODHA") else 1
        bid = _batch26o16g_float(raw.get("bid") or raw.get("best_bid"), 0.0)
        ask = _batch26o16g_float(raw.get("ask") or raw.get("best_ask"), 0.0)
...
def _batch26o16g_merge_selected(selected: Mapping[str, Any], source: Mapping[str, Any]) -> dict[str, Any]:
    out = dict(selected or {})
    if not source:
        return out

    side_raw = str(source.get("option_side") or source.get("instrument_role") or out.get("side") or "").upper()
    side = "CALL" if ("CALL" in side_raw or "CE" in side_raw) else ("PUT" if ("PUT" in side_raw or "PE" in side_raw) else out.get("side"))

    bid = _batch26o16g_float(source.get("bid") or source.get("best_bid") or out.get("best_bid"), 0.0)
    ask = _batch26o16g_float(source.get("ask") or source.get("best_ask") or out.get("best_ask"), 0.0)
    ltp = _batch26o16g_float(source.get("ltp") or source.get("last_price") or out.get("ltp"), 0.0)

    if source.get("instrument_key"):
        out["instrument_key"] = source.get("instrument_key")
    if source.get("instrument_token"):
        out["instrument_token"] = str(source.get("instrument_token"))
        out["option_token"] = str(source.get("instrument_token"))
    if source.get("trading_symbol"):
        out["trading_symbol"] = source.get("trading_symbol")
        out["option_symbol"] = source.get("trading_symbol")
    if source.get("strike"):
        out["strike"] = _batch26o16g_float(source.get("strike"), 0.0) or out.get("strike")
    if source.get("expiry"):
        out["expiry"] = source.get("expiry")
    if side:
        out["side"] = side
        out["option_side"] = side
    if source.get("provider_id"):
        out["provider_id"] = str(source.get("provider_id")).upper()

    out["ltp"] = ltp or out.get("ltp")
    out["best_bid"] = bid or out.get("best_bid")
    out["best_ask"] = ask or out.get("best_ask")

    if bid > 0 and ask > 0:
        spread = max(0.0, ask - bid)
        out["spread"] = spread
        out["spread_ratio"] = spread / max(bid, 0.05)
        out["mid"] = (bid + ask) / 2.0

    bid_qty = int(_batch26o16g_float(source.get("bid_qty") or out.get("bid_qty_5"), 0.0))
    ask_qty = int(_batch26o16g_float(source.get("ask_qty") or out.get("ask_qty_5"), 0.0))
    out["bid_qty_5"] = bid_qty
    out["ask_qty_5"] = ask_qty
    out["depth_total"] = int((bid_qty or 0) + (ask_qty or 0)) or int(_batch26o16g_float(out.get("depth_total"), 0.0))

    tick_validity = str(source.get("tick_validity") or out.get("tick_validity") or "").upper()
    reject_reason = str(source.get("reject_reason") or out.get("reject_reason") or "")
    out["tick_validity"] = tick_validity
    out["reject_reason"] = reject_reason
    out["anomaly_clamped"] = bool(tick_validity == "ANOMALY_CLAMPED" or reject_reason)
    out["present"] = bool(out.get("instrument_key") or out.get("ltp"))
    out["quote_present"] = bool((ltp and ltp > 0) or (bid > 0 and ask > 0))
    out["book_present"] = bool(source.get("bids") or source.get("asks") or (bid > 0 and ask > 0))
    out["depth_ok"] = bool(out.get("depth_total", 0) and int(out.get("depth_total", 0)) > 0)
    ...
    tradability_ok = bool(
        ...
        and out["depth_ok"]
        and not out["anomaly_clamped"]
        and (bid <= ask if bid and ask else True)
    )
    ...
    out["source_bridge"] = "batch26o16g"
    out["raw_source"] = dict(source)
    return out


if "_BATCH26O16G_ORIGINAL_FEATURESERVICE_RUN_ONCE" not in globals():
    _BATCH26O16G_ORIGINAL_FEATURESERVICE_RUN_ONCE = FeatureService.run_once

    def _batch26o16g_run_once(self: FeatureService, *args: Any, **kwargs: Any) -> dict[str, Any]:
        payload = dict(_BATCH26O16G_ORIGINAL_FEATURESERVICE_RUN_ONCE(self, *args, **kwargs))
        ...
        selected_side = str(selected.get("side") or selected.get("option_side") or "").upper()
        source = _batch26o16g_best_selected_source(self.redis, selected_side=selected_side)
        selected = _batch26o16g_merge_selected(selected, source)
        ...
        if provider_id in {getattr(N, "PROVIDER_ZERODHA", "ZERODHA"), getattr(N, "PROVIDER_DHAN", "DHAN")}:
            # Classic families may run with Zerodha-selected option bridge; Dhan data
            # may support source evidence but does not enable MISO unless Dhan context is fresh.
    ...
    FeatureService.run_once = _batch26o16g_run_once
'''
    TARGET_FEATURES.write_text(before_text.rstrip() + "\n" + patch + "\n", encoding="utf-8")
    return {"patched": True, "already_present": False, "backup": str(backup), "reason": "selected-option provider/data-quality/tradability bridge added"}
...
def main() -> int:
    result: dict[str, Any] = {
        "batch": BATCH,
        "batch_name": "selected_option_provider_data_quality_tradability",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "scope": {
            "guarded": True,
            "paper_restart": False,
            ...
            "allowed_patch": "features.py selected-option provider/data-quality/tradability mapping only",
        },
    }
    ...
    o16f = safe_json_load(O16F_PATH.read_text(encoding="utf-8")) if O16F_PATH.exists() else {}
    result["o16f_gate"] = {
        "exists": O16F_PATH.exists(),
        "final_verdict": o16f.get("final_verdict") if isinstance(o16f, Mapping) else None,
        "before_summary": o16f.get("before_summary") if isinstance(o16f, Mapping) else None,
        "required_verdicts": o16f.get("required_verdicts") if isinstance(o16f, Mapping) else None,
    }

    if o16f.get("final_verdict") != "FAIL_O16F_SELECTED_OPTION_MAPPING_NOT_PROVEN":
        result["final_verdict"] = "FAIL_CLOSED_O16F_NOT_EXPECTED_VERDICT"
        result["patch_performed"] = False
        write_outputs(result)
        return 2
    ...
    exact_patch_allowed = bool(
        before_flags.get("futures_present") is True
    ...
    if exact_patch_allowed:
        patch_result = patch_features_provider_quality_bridge(before_text)
    else:
        patch_result = {"patched": False, "already_present": False, "reason": "exact O16G patch gate not satisfied"}
    ...
    post_script = ROOT / "run/proofs/_tmp_batch26o16g_post_check.py"
...
quality = json.loads(raw.get("o16g_quality_json") or "{}")
...
out = {
    ...
    "o16g_quality_present": bool(quality),
    "o16g_quality": quality,
}
...
''', encoding="utf-8")
    ...
    required = {
        ...
        "o16f_gate_expected": result["o16f_gate"]["final_verdict"] == "FAIL_O16F_SELECTED_OPTION_MAPPING_NOT_PROVEN",
    ...
    if not all([
        ...
        result["final_verdict"] = "FAIL_O16G_PROVIDER_DATA_QUALITY_REPAIR_NOT_PROVEN"
    ...
    if (
        ...
        result["final_verdict"] = "PASS_O16G_RUNTIME_DATA_VALID_SAFE_TO_CONSUME_OK"
        result["next_recommended_batch"] = "26-O17 activation candidate extraction proof, no risk/execution"
        write_outputs(result)
        return 0

    result["final_verdict"] = "PASS_O16G_PROVIDER_DATA_QUALITY_REPAIRED_DATA_VALID_STILL_FAIL_CLOSED"
