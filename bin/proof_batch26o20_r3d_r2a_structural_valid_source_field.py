...
BATCH = "26-O20-R3D-R2A"
PROOF_PATH = ROOT / "run/proofs/proof_batch26o20_r3d_r2a_structural_valid_source_field.json"
MANIFEST_PATH = ROOT / "run/proofs/manifest_batch26o20_r3d_r2a_structural_valid_source_field.json"
R2_PATH = ROOT / "run/proofs/proof_batch26o20_r3d_r2_source_publish_repair.json"
TARGET_FEATURES = ROOT / "app/mme_scalpx/services/features.py"
RUN_DIR = ROOT / "run/live_capture/batch26o20_r3d_r2a_structural_valid_source_field"
...
def pgrep_service(service: str) -> list[str]:
    matches: list[str] = []
    self_name = "proof_batch26o20_r3d_r2a_structural_valid_source_field.py"
...
def patch_features_structural_valid(before: str) -> dict[str, Any]:
    marker = "Batch 26-O20-R3D-R2A structural_valid source-field repair"
    if marker in before:
        return {"patched": False, "already_present": True, "reason": "marker already present"}

    if "Batch 26-O20-R3D-R2 source-level consumer-view publish repair" not in before:
        return {"patched": False, "already_present": False, "reason": "R2 marker missing"}

    backup = ROOT / "run/_code_backups" / f"batch26o20_r3d_r2a_features_py_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pre_patch"
    backup.parent.mkdir(parents=True, exist_ok=True)
    backup.write_text(before, encoding="utf-8")

    patched = before

    # Correct the R2 helper so structural_valid is present whenever data_valid
    # and safe_to_consume are made true. Also tolerate pre-existing truthy
    # data_valid/safe_to_consume when branch structure is complete.
    old = '''        out["data_valid"] = True
        out["safe_to_consume"] = True
        out["structural_valid"] = True
        out["hold_only"] = True'''
    new = '''        out["data_valid"] = True
        out["safe_to_consume"] = True
        out["structural_valid"] = True
    ...
    if old in patched:
        ...
        applied.append("r2_helper_structural_valid_expansion")

    # If a later publish path bypasses the helper but already sets data_valid /
    # safe_to_consume true, normalize the serialized consumer_view dict just
    # before JSON dump using a tiny source-level normalizer.
    helper = r'''
...
'''
    ...
    # Also patch any remaining direct publish sites that serialize consumer_view.
    patched2, n1 = re.subn(
        r'(["\']consumer_view_json["\']\s*:\s*_json_dump\()consumer_view(\)\s*,)',
        r'\1_batch26o20r3d_r2a_force_structural_valid(consumer_view)\2',
        patched,
        count=4,
    )
    patched = patched2
    if n1:
        applied.append(f"regex_consumer_view_json_remaining:{n1}")

    patched2, n2 = re.subn(
        r'(["\']payload_json["\']\s*:\s*_json_dump\()payload(\)\s*,)',
        r'\1_batch26o20r3d_r2a_force_payload_structural_valid(payload)\2',
        patched,
        count=4,
    )
    patched = patched2
    if n2:
        applied.append(f"regex_payload_json_remaining:{n2}")

    if not applied:
        return {"patched": False, "already_present": False, "reason": "no structural_valid source pattern matched", "backup": str(backup)}
...
def feature_snapshot(client: Any, features_hash_key: str) -> dict[str, Any]:
    ...
    sem_r2 = hget_json(client, features_hash_key, "o20r3d_r2_validity_semantics_json")
    sem_r2a = hget_json(client, features_hash_key, "o20r3d_r2a_structural_valid_json")
    ...
    return {
        ...
        "consumer_view_structural_valid_alias": cv.get("consumer_view_structural_valid") if isinstance(cv, Mapping) else None,
        ...
        "payload_consumer_view_structural_valid_alias": payload_cv.get("consumer_view_structural_valid") if isinstance(payload_cv, Mapping) else None,
        ...
        "r2_semantics_guard_present": bool(sem_r2),
        "r2a_structural_valid_guard_present": bool(sem_r2a),
        "r2a_structural_valid_guard": sem_r2a,
    }
...
def run_feature_once(client: Any) -> dict[str, Any]:
    try:
        ...
        svc = F.FeatureService(
            ...
            instance_id="batch26o20-r3d-r2a-feature",
        )
...
def start_features_service() -> dict[str, Any]:
    ...
    log_path = RUN_DIR / "o20_r3d_r2a_features.log"
    pid_path = RUN_DIR / "o20_r3d_r2a_features.pid"
    args = [
        sys.executable,
        "-m",
        "app.mme_scalpx.main",
...
def run_strategy_once(client: Any) -> dict[str, Any]:
    try:
        ...
        svc = S.StrategyService(
            ...
            clock=type("Clock", (), {"now_ns": staticmethod(time.time_ns)})(),
            shutdown=type("S", (), {"is_set": staticmethod(lambda: True)})(),
            instance_id="batch26o20-r3d-r2a-strategy",
        )
        res = svc.run_once()
...
def main() -> int:
    result: dict[str, Any] = {
        "batch": BATCH,
        "batch_name": "structural_valid_source_field",
        "created_at_utc": now_utc(),
        "scope": {
    ...
    result["r2_gate"] = {
        "exists": bool(r2),
        "final_verdict": r2.get("final_verdict"),
        "required_verdicts": r2.get("required_verdicts"),
    }

    if r2.get("final_verdict") != "FAIL_O20_R3D_R2_SOURCE_PUBLISH_REPAIR_NOT_PROVEN":
        result["final_verdict"] = "FAIL_CLOSED_R2_NOT_EXPECTED_FAILURE"
    ...
    patch_result = patch_features_structural_valid(before_text)
    ...
    sample_structural_alias_count = sum(1 for s in samples if s["feature_snapshot"].get("consumer_view_structural_valid_alias") is True)
    sample_payload_structural = sum(1 for s in samples if s["feature_snapshot"].get("payload_consumer_view_structural_valid") is True)
    ...
    required = {
        ...
        "r2_expected_fail_gate": r2.get("final_verdict") == "FAIL_O20_R3D_R2_SOURCE_PUBLISH_REPAIR_NOT_PROVEN",
        ...
        "feature_once_payload_cv_safe": feature_once.get("payload_consumer_view_safe_to_consume") is True,
        "feature_once_payload_structural_valid": feature_once.get("payload_consumer_view_structural_valid") is True,
        ...
        "snapshot_after_once_structural_valid_alias_true": snapshot_after_once.get("consumer_view_structural_valid_alias") is True,
        "snapshot_after_once_payload_structural_valid_true": snapshot_after_once.get("payload_consumer_view_structural_valid") is True,
        ...
        "r2a_structural_valid_guard_present": snapshot_after_once.get("r2a_structural_valid_guard_present") is True,
        ...
        "consumer_view_structural_valid_alias_in_samples": sample_structural_alias_count >= 4,
        "payload_structural_valid_in_samples": sample_payload_structural >= 4,
    ...
    if not all(required.values()):
        result["final_verdict"] = "FAIL_O20_R3D_R2A_STRUCTURAL_VALID_SOURCE_FIELD_NOT_PROVEN"
        result["next_recommended_batch"] = "Inspect R2A proof. Do not run O20-R3E."
        write_outputs(result)
        return 2

    result["final_verdict"] = "PASS_O20_R3D_R2A_STRUCTURAL_VALID_SOURCE_FIELD_OK_HOLD_ONLY"
