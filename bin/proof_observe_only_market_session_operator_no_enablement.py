...
SCAN_FILES = [
    Path("app/mme_scalpx/replay/live_capture_runbook.py"),
    Path("bin/observe_only_market_session_capture_operator.py"),
]
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                if any(x in alias.name.lower() for x in FORBIDDEN_IMPORTS):
                    imports.append({"line": node.lineno, "import": alias.name})
        elif isinstance(node, ast.ImportFrom):
            module = (node.module or "").lower()
            if any(x in module for x in FORBIDDEN_IMPORTS):
...
def main() -> int:
    ...
    parser.add_argument("--out", default="run/proofs/proof_observe_only_market_session_operator_no_enablement.json")
    args = parser.parse_args()

    runbook = load_json(Path("run/proofs/proof_observe_only_market_session_capture_runbook.json"))
    ...
    static_ok = all(item["ok"] for item in scans)

    ok = bool(
        static_ok
        and runbook.get("observe_only_market_session_capture_runbook_ok") is True
    ...
    proof = {
        "schema_version": "proof_observe_only_market_session_operator_no_enablement_v1",
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "observe_only_market_session_operator_no_enablement_ok": ok,
        "static_scan_ok": static_ok,
        "scan_results": scans,
        ...
        "verdict": "PASS_OBSERVE_ONLY_MARKET_SESSION_OPERATOR_NO_ENABLEMENT" if ok else "FAIL_REVIEW_REQUIRED",
    }
