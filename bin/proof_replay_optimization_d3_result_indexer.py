...
ARTIFACT_ROOT = ROOT / "run" / "replay_optimization" / f"d3_replay_index_probe_{TS}"
...
EXPECTED_FILES = [
    ...
    "etc/replay_optimization/indexers/replay_result_index_contract.json",
...
if missing:
    raise SystemExit(f"missing expected D3 files: {missing}")
...
indexer = importlib.import_module("app.mme_scalpx.replay_optimization.result_indexer")

config = json.loads(files["etc/replay_optimization/indexers/replay_result_index_contract.json"].read_text(encoding="utf-8"))
if config["safety"]["replay_execution_allowed"] is not False:
    raise SystemExit("D3 config replay execution safety failed")
if config["safety"]["ml_training_allowed"] is not False:
    raise SystemExit("D3 config ML training safety failed")
if config["safety"]["broker_calls_allowed"] is not False:
    raise SystemExit("D3 config broker safety failed")
if config["safety"]["live_redis_writes_allowed"] is not False:
    raise SystemExit("D3 config Redis safety failed")
if config["safety"]["paper_live_enablement_allowed"] is not False:
    raise SystemExit("D3 config paper/live safety failed")

optimization_id = f"D3_REPLAY_INDEX_PROBE_{TS}"
index = indexer.build_replay_input_index(
    optimization_id,
    ("run/replay",),
    max_files=5000,
)
index_path = indexer.write_replay_input_index(index, ARTIFACT_ROOT)
rows = indexer.replay_result_ref_rows_from_index(index)
summary = indexer.result_indexer_summary(("run/replay",))

if not Path(index_path).exists():
    raise SystemExit("D3 replay input index was not written")

if index.replay_execution_performed is not False:
    raise SystemExit("D3 unexpectedly performed replay execution")
if index.ml_training_performed is not False:
    raise SystemExit("D3 unexpectedly performed ML training")
if index.broker_calls_executed is not False:
    raise SystemExit("D3 unexpectedly executed broker calls")
if index.live_redis_writes_executed is not False:
    raise SystemExit("D3 unexpectedly wrote live Redis")
if index.paper_or_live_enabled is not False:
    raise SystemExit("D3 unexpectedly enabled paper/live")

for row in rows[:50]:
    if not isinstance(row, contracts.ReplayResultRefRow):
        raise SystemExit("D3 replay result ref row type mismatch")

file_hashes = {rel: sha256(path) for rel, path in files.items()}

proof = {
    "batch": "LANE-D-D3",
    "name": "read_only_replay_result_indexer_contract",
    "created_at": NOW,
    "verdict": "PASS",
    "accepted_for": "READ_ONLY_INDEX_CONTRACT",
    "d2_dependency_observed": True,
    "files_checked": EXPECTED_FILES,
    "file_hashes": file_hashes,
    "compile_results": compile_results,
    "import_audit": import_audit,
    "artifact_root": ARTIFACT_ROOT.as_posix(),
    "replay_input_index_path": index_path.as_posix(),
    "artifact_count": index.artifact_count,
    "artifact_kinds": list(index.artifact_kinds),
    "replay_result_ref_row_count": len(rows),
    ...
    "next_recommended_batch": "LANE-D-D4_READ_ONLY_RAW_RESEARCH_FEATURE_INDEXER_CONTRACT"
}

proof_path = PROOF_DIR / f"proof_lane_d_d3_replay_result_indexer_{TS}.json"
latest_path = PROOF_DIR / "proof_lane_d_d3_replay_result_indexer_latest.json"
proof_path.write_text(json.dumps(proof, indent=2, sort_keys=True), encoding="utf-8")
shutil.copy2(proof_path, latest_path)

milestone_path = MILESTONE_DIR / f"lane_d_d3_replay_result_indexer_{TS}.md"
