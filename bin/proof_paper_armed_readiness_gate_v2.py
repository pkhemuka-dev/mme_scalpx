...
OUT = ROOT / "run/proofs/proof_paper_armed_readiness_gate_v2.json"
...
def main() -> int:
    ...
    add(
        "strong_replay_sample_pass",
        is_pass(sr)
        and sr.get("replay_only_channels") is True
        and int(sr.get("events_materialized") or 0) >= int(sr.get("minimum_events_required") or 150)
        and len(sr.get("kinds") or []) >= int(sr.get("minimum_kinds_required") or 3),
        "P1",
        f"status={status(sr)} events={sr.get('events_materialized')} kinds={sr.get('kinds')} path={sr.get('_path')}",
    )

    sem = proofs["semantic_replay_dry_run"]
    add(
        "semantic_replay_dry_run_pass",
    ...
    result = {
        "proof": "proof_paper_armed_readiness_gate_v2",
        ...
        "notes": [
            ...
            "This v2 gate requires a strong replay sample, not just the 3-event semantic smoke.",
