...
CONTROL = Path("run/controls/pshadowgate_lifecycle_v21.env")
STATE_DIR = Path("run/state/pshadowgate_lifecycle_v21")
LEDGER_DIR = Path("run/paper_shadow_lifecycle_v21")
...
def load_cfg() -> dict[str, Any]:
    ...
    return {
        ...
        "abs_stop_points": cfg_float(c, "ABS_STOP_POINTS", 5.0),
        "hard_target_points": cfg_float(c, "HARD_TARGET_POINTS", 8.0),
        "max_holding_time_ms": cfg_int(c, "MAX_HOLDING_TIME_MS", 180000),
        "breakeven_enable": cfg_bool(c, "BREAKEVEN_ENABLE", True),
        "breakeven_after_mfe_points": cfg_float(c, "BREAKEVEN_AFTER_MFE_POINTS", 2.0),
        "breakeven_exit_buffer_points": cfg_float(c, "BREAKEVEN_EXIT_BUFFER_POINTS", 0.20),
        "trailing_enable": cfg_bool(c, "TRAILING_ENABLE", True),
        "trailing_activate_mfe_points": cfg_float(c, "TRAILING_ACTIVATE_MFE_POINTS", 2.5),
        "trailing_giveback_points": cfg_float(c, "TRAILING_GIVEBACK_POINTS", 1.25),
        "stale_ltp_exit_enable": cfg_bool(c, "STALE_LTP_EXIT_ENABLE", True),
        "stale_ltp_grace_ms": cfg_int(c, "STALE_LTP_GRACE_MS", 60000),
    }
...
def latest_pshadow_event(d: str) -> dict[str, Any] | None:
...
def already_entered(d: str, event_id: str) -> bool:
    return any(str(x.get("event_id")) == event_id for x in read_ndjson(entries_path(d)))
...
def write_summary(d: str) -> dict[str, Any]:
    ...
    for x in exits:
        ...
        reason = str(x.get("exit_reason") or "UNKNOWN")
...
def enter_if_possible() -> dict[str, Any]:
    ...
    pos = {
        "schema": "pshadow_lifecycle_v21_open_position.v1",
        "status": "OPEN",
        "event_id": event_id,
...
def update_or_exit() -> dict[str, Any]:
    ...
    exit_obj = {
        "schema": "pshadow_lifecycle_v21_exit.v1",
...
def run_once() -> dict[str, Any]:
    ...
    result.setdefault("final_classification", "PSHADOW_LIFECYCLE_V21_ONCE_COMPLETE_NO_BROKER_ORDER")
...
def main() -> int:
    ...
    print("V21_ACTION=" + str(result.get("action")))
