...
- deterministic empty builders for payloads and family support blocks
- thin shape validation helpers for feature-family publication
...
- missing data must be explicit via False / None, never by omitting required keys
...
FAMILY_FEATURES_VERSION: Final[str] = "1.0"

# ============================================================================
# Frozen family / branch identity anchored to names.py
# ============================================================================
...
CLASSIC_BRANCH_COMMON_KEYS: Final[tuple[str, ...]] = (
    "context_pass",
...
def validate_provider_runtime_block(provider_runtime: Mapping[str, Any]) -> None:
    _require_exact_keys(
        provider_runtime,
...
def validate_family_features_payload(payload: Mapping[str, Any]) -> None:
    ...
    _require(
        schema_version >= 1,
        f"payload.schema_version must be >= 1, got {schema_version}",
    )
    _require(
        family_features_version == FAMILY_FEATURES_VERSION,
