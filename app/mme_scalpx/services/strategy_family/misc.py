...
Freeze-grade MISC doctrine leaf evaluator for ScalpX MME.

Purpose
-------
This module OWNS:
- pure MISC CALL/PUT candidate evaluation from the already-built strategy
  consumer view / family feature surfaces
- deterministic no-signal / blocked / candidate result construction
- MISC-specific compression -> breakout -> expansion -> retest/hesitation ->
  resume-confirmation support checks
...
Frozen MISC identity
--------------------
MISC is the compression-breakout sibling strategy.
...
- Dhan-enhanced
- degraded-safe
- compression-box -> directional burst -> monitored retest/hesitation -> resume
...
DOCTRINE_ID: Final[str] = getattr(N, "DOCTRINE_MISC", "MISC")
...
MIN_SCORE_NORMAL: Final[float] = 0.58
MIN_SCORE_FAST: Final[float] = 0.62
MIN_SCORE_LOWVOL: Final[float] = 0.66

MIN_COMPRESSION_SCORE: Final[float] = 0.45
MIN_BREAKOUT_SCORE: Final[float] = 0.52
MIN_RETEST_RESUME_SCORE: Final[float] = 0.48
MIN_OPTION_CONFIRMATION: Final[float] = 0.48
MIN_CONTEXT_SCORE: Final[float] = 0.42

OI_WALL_VETO_DISTANCE_STRIKES: Final[float] = 0.65
OI_WALL_STRONG_MIN: Final[float] = 0.74
...
class MiscCandidate:
    ...
    setup_kind: str = "MISC_COMPRESSION_BREAKOUT_RETEST_RESUME"
    retest_type: str | None = None
    breakout_ref: float | None = None
    retest_ref: float | None = None
...
class MiscEvaluationResult:
    ...
    is_candidate: bool
    is_blocked: bool
    candidate: MiscCandidate | None = None
    blocker: MiscBlocker | None = None
    metadata: Mapping[str, Any] = field(default_factory=dict)
...
) -> MiscEvaluationResult:
    return MiscEvaluationResult(
        family_id=FAMILY_ID,
...
def candidate_result(candidate: MiscCandidate) -> MiscEvaluationResult:
    return MiscEvaluationResult(
        family_id=FAMILY_ID,
        doctrine_id=DOCTRINE_ID,
        ...
        candidate=candidate,
        blocker=None,
        metadata={"reason": "misc_candidate_ready"},
    )
...
def directional_breakout_ok(view: Mapping[str, Any], branch_id: str, surface: Mapping[str, Any]) -> bool:
    if bool_any(surface, ("directional_breakout_ok", "breakout_trigger_ok", "breakout_triggered")):
        return True
    ...
    if branch_id == BRANCH_CALL:
        return bool(ofi >= 0.0 and delta_3 > 0.0 and (above_vwap or delta_3 >= 2.0))
    return bool(ofi <= 0.0 and delta_3 < 0.0 and (below_vwap or delta_3 <= -2.0))


def compression_score(surface: Mapping[str, Any]) -> float:
    explicit = float_any(surface, ("compression_score", "compression_quality_score"), -1.0)
    if explicit >= 0.0:
        return clamp(explicit, 0.0, 1.0)

    valid = bool_any(surface, ("compression_detected", "compression_valid", "compression_ok"))
    cluster = float_any(surface, ("compression_cluster_score", "cluster_score"), 0.0)
    width_ok = bool_any(surface, ("compression_width_ok", "bb_width_ok", "box_width_ok"))
    prebreak = bool_any(surface, ("prebreak_proximity_ok", "near_box_edge"))
    ...
    if prebreak:
        score += 0.20
    score += clamp(cluster, 0.0, 1.0) * 0.15
    return clamp(score, 0.0, 1.0)


def breakout_score(view: Mapping[str, Any], branch_id: str, surface: Mapping[str, Any]) -> float:
    explicit = float_any(surface, ("breakout_score", "breakout_quality_score"), -1.0)
    if explicit >= 0.0:
        return clamp(explicit, 0.0, 1.0)
    ...
    trigger = directional_breakout_ok(view, branch_id, surface)
    expansion = bool_any(surface, ("expansion_acceptance_ok", "breakout_accepted", "expansion_accepted"))
    ...
    if expansion:
        score += 0.25
    score += clamp((velocity - 0.80) / 1.30, 0.0, 1.0) * 0.20
    score += clamp((vol_norm - 0.80) / 1.30, 0.0, 1.0) * 0.10
    score += clamp(ofi_abs, 0.0, 1.0) * 0.10
    return clamp(score, 0.0, 1.0)


def retest_resume_score(surface: Mapping[str, Any]) -> tuple[float, str | None]:
    explicit = float_any(surface, ("retest_resume_score", "resume_score"), -1.0)
    if explicit >= 0.0:
    ...
    return clamp(score, 0.0, 1.0), retest_type
...
def option_confirmation_score(
    ...
    explicit = float_any(surface, ("option_confirmation_score", "option_confirm_score"), -1.0)
    if explicit >= 0.0:
        return clamp(explicit, 0.0, 1.0)
    ...
    return clamp(
        ...
        + (0.25 if tradability_ok else 0.0)
        + min(delta, 3.0) / 3.0 * 0.20,
        0.0,
        1.0,
    )
...
def context_score(
    ...
    base = clamp(explicit, 0.0, 1.0) if explicit >= 0.0 else 0.62
    ...
    if distance <= OI_WALL_VETO_DISTANCE_STRIKES and strength >= OI_WALL_STRONG_MIN:
        return max(0.0, base - 0.40), False, "near_strong_oi_wall"
...
def compute_score(
    *,
    ...
    comp = compression_score(surface)
    brk = breakout_score(view, branch_id, surface)
    retest, retest_type = retest_resume_score(surface)
    opt = option_confirmation_score(view, branch_id, surface, frame)
    ...
    return score, {
        ...
        "breakout_score": brk,
        "retest_resume_score": retest,
        "option_confirmation_score": opt,
        "context_score": ctx,
...
def evaluate_branch(view_like: Any, branch_id: str) -> MiscEvaluationResult:
    ...
    family_status = as_mapping(view.get("family_status"))
    misc_status = as_mapping(family_status.get(FAMILY_ID))
    if misc_status and not safe_bool(misc_status.get("family_present"), False):
        return no_signal_result(
            branch_id=branch_id,
            reason="misc_family_not_present",
        )
    ...
    if not directional_breakout_ok(view, branch_id, surface):
        return no_signal_result(
            branch_id=branch_id,
    ...
    if not degraded_ok:
        return blocked_result(
            ...
            message="MISC degraded mode allows ATM only.",
    ...
    if score_parts["compression_score"] < MIN_COMPRESSION_SCORE:
        return no_signal_result(
            branch_id=branch_id,
    ...
    if score_parts["breakout_score"] < MIN_BREAKOUT_SCORE:
        return no_signal_result(
            branch_id=branch_id,
            reason="breakout_insufficient",
            metadata=score_parts,
        )

    if score_parts["retest_resume_score"] < MIN_RETEST_RESUME_SCORE:
        return no_signal_result(
            branch_id=branch_id,
            reason="retest_resume_insufficient",
            metadata=score_parts,
        )
    ...
    candidate = MiscCandidate(
        ...
        retest_type=safe_str(score_parts.get("retest_type")) or None,
        breakout_ref=safe_float(surface.get("breakout_ref"), 0.0) if surface.get("breakout_ref") not in (None, "") else None,
        retest_ref=safe_float(surface.get("retest_ref"), 0.0) if surface.get("retest_ref") not in (None, "") else None,
        ...
        metadata={
            ...
            "setup_kind": "compression_breakout_retest_resume",
        },
    )
    return candidate_result(candidate)


def evaluate(view_like: Any, branch_id: str | None = None) -> MiscEvaluationResult:
    ...
    return no_signal_result(
        branch_id=None,
        reason="no_misc_branch_candidate",
    )


def evaluate_family(view_like: Any) -> MiscEvaluationResult:
    return evaluate(view_like)


def evaluate_doctrine(view_like: Any, branch_id: str | None = None) -> MiscEvaluationResult:
