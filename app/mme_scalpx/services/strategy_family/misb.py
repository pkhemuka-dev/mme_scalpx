...
Freeze-grade MISB doctrine leaf evaluator for ScalpX MME.

Purpose
-------
This module OWNS:
- pure MISB CALL/PUT candidate evaluation from the already-built strategy
  consumer view / family feature surfaces
- deterministic no-signal / blocked / candidate result construction
- MISB-specific shelf-breakout-acceptance support checks
...
Frozen MISB identity
--------------------
MISB is the breakout / shelf-break member of the MIS family.
...
FAMILY_ID: Final[str] = getattr(N, "STRATEGY_FAMILY_MISB", "MISB")
DOCTRINE_ID: Final[str] = getattr(N, "DOCTRINE_MISB", "MISB")
...
MIN_SCORE_NORMAL: Final[float] = 0.56
MIN_SCORE_FAST: Final[float] = 0.60
MIN_SCORE_LOWVOL: Final[float] = 0.64

MIN_FUTURES_BIAS: Final[float] = 0.48
MIN_BREAKOUT_SCORE: Final[float] = 0.54
...
def _batch26d_required_common_surface(container, key, *, family="MISB", source="unknown"):
    """
...
class MisbCandidate:
    ...
    setup_kind: str = "MISB_SHELF_BREAKOUT_ACCEPTANCE"
...
class MisbEvaluationResult:
    ...
    is_candidate: bool
    is_blocked: bool
    candidate: MisbCandidate | None = None
    blocker: MisbBlocker | None = None
    metadata: Mapping[str, Any] = field(default_factory=dict)
...
) -> MisbEvaluationResult:
    return MisbEvaluationResult(
        family_id=FAMILY_ID,
...
def candidate_result(candidate: MisbCandidate) -> MisbEvaluationResult:
    return MisbEvaluationResult(
        family_id=FAMILY_ID,
        doctrine_id=DOCTRINE_ID,
        ...
        is_blocked=False,
        candidate=candidate,
        metadata={"reason": "misb_candidate_ready"},
    )
...
def extract_provider_runtime(view: Mapping[str, Any]) -> dict[str, Any]:
    return as_mapping(_batch26d_required_common_surface(view, "provider_runtime", family="MISB", source="view"))
...
def selected_option(view: Mapping[str, Any], branch_id: str) -> dict[str, Any]:
    common = extract_common(view)
    selected = as_mapping(_batch26d_required_common_surface(common, "selected_option", family="MISB", source="common"))
...
def futures_bias_ok(view: Mapping[str, Any], branch_id: str, surface: Mapping[str, Any]) -> bool:
    if bool_any(surface, ("futures_bias_ok", "bias_ok", "direction_ok")):
        return True
    ...
    if branch_id == BRANCH_CALL:
        score = (1.0 if ofi >= 0.0 else 0.0) * 0.35 + (1.0 if delta_3 >= 0.0 else 0.0) * 0.35 + (1.0 if above_vwap or velocity >= 1.0 else 0.0) * 0.30
    else:
        score = (1.0 if ofi <= 0.0 else 0.0) * 0.35 + (1.0 if delta_3 <= 0.0 else 0.0) * 0.35 + (1.0 if below_vwap or velocity >= 1.0 else 0.0) * 0.30

    return score >= MIN_FUTURES_BIAS


def breakout_trigger_score(view: Mapping[str, Any], surface: Mapping[str, Any]) -> float:
    explicit = float_any(surface, ("breakout_score", "breakout_trigger_score"), -1.0)
    if explicit >= 0.0:
        return clamp(explicit, 0.0, 1.0)

    triggered = bool_any(surface, ("breakout_triggered", "breakout_trigger_ok", "triggered"))
    accepted = bool_any(surface, ("breakout_accepted", "breakout_acceptance_ok", "accepted"))
    shelf_valid = bool_any(surface, ("shelf_confirmed", "breakout_shelf_valid", "shelf_valid", "shelf_ok", "base_valid"))
    ...
    if accepted:
        score += 0.28
    score += clamp((velocity - 0.85) / 1.30, 0.0, 1.0) * 0.10
    score += clamp((vol_norm - 0.85) / 1.30, 0.0, 1.0) * 0.04
    score += clamp(ofi_abs, 0.0, 1.0) * 0.02
...
def option_confirmation_score(
    ...
    return clamp(
        ...
        + (0.28 if tradability_ok else 0.0)
...
def compute_score(
    *,
    ...
    breakout = breakout_trigger_score(view, surface)
    ...
    score = clamp(
        ...
        + option * 0.33
        + ctx_score * 0.15
        + (0.10 if futures_bias_ok(view, branch_id, surface) else 0.0),
        0.0,
        1.0,
    )
    ...
    return score, {
        ...
        "futures_bias_ok": futures_bias_ok(view, branch_id, surface),
    }
...
def evaluate_branch(view_like: Any, branch_id: str) -> MisbEvaluationResult:
    ...
    family_status = as_mapping(view.get("family_status"))
    misb_status = as_mapping(family_status.get(FAMILY_ID))
    if misb_status and not safe_bool(misb_status.get("family_present"), False):
        return no_signal_result(
            branch_id=branch_id,
            reason="misb_family_not_present",
        )
    ...
    if not futures_bias_ok(view, branch_id, surface):
        return no_signal_result(
            branch_id=branch_id,
            reason="futures_bias_not_confirmed",
        )
    ...
    if not degraded_ok:
        return blocked_result(
            ...
            message="MISB degraded mode allows ATM only.",
    ...
    if score_parts["breakout_score"] < MIN_BREAKOUT_SCORE:
        return no_signal_result(
            branch_id=branch_id,
            reason="breakout_score_insufficient",
            metadata=score_parts,
        )
    ...
    candidate = MisbCandidate(
        ...
        metadata={
            ...
            "setup_kind": "shelf_breakout_acceptance",
        },
    )
    return candidate_result(candidate)


def evaluate(view_like: Any, branch_id: str | None = None) -> MisbEvaluationResult:
    ...
    return no_signal_result(
        branch_id=None,
        reason="no_misb_branch_candidate",
    )


def evaluate_family(view_like: Any) -> MisbEvaluationResult:
    return evaluate(view_like)


def evaluate_doctrine(view_like: Any, branch_id: str | None = None) -> MisbEvaluationResult:
...
_BATCH26_OI_C_FAMILY_ID = "MISB"
_BATCH26_OI_C_SOFT_POLICY = "wall_break_acceptance_context_only"
_BATCH26_OI_C_SOFT_POLICY_DESCRIPTION = "MISB uses OI wall only as breakout/wall-acceptance context."
