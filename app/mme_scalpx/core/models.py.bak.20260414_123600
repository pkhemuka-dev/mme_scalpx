...
class InstrumentRole:
    ...
    CALL_SELECTED: Final[str] = "CALL_SELECTED"
    PUT_SELECTED: Final[str] = "PUT_SELECTED"


ALLOWED_INSTRUMENT_ROLES: Final[tuple[str, ...]] = (
    InstrumentRole.FUTURES,
    InstrumentRole.CALL_SELECTED,
    InstrumentRole.PUT_SELECTED,
)
...
class SnapshotValidity:
    ...
    OUT_OF_SYNC: Final[str] = "OUT_OF_SYNC"
    STALE: Final[str] = "STALE"
    REJECTED: Final[str] = "REJECTED"
...
ALLOWED_SNAPSHOT_VALIDITY: Final[tuple[str, ...]] = (
    ...
    SnapshotValidity.OUT_OF_SYNC,
    SnapshotValidity.STALE,
    SnapshotValidity.REJECTED,
)
...
class FeedTick(SchemaBase):
    ...
    tick_validity: str = TickValidity.VALID
...
class SnapshotFrame(SchemaBase):
    ...
    selected_call: OptionSnapshot | None = None
    selected_put: OptionSnapshot | None = None
    ...
    def validate(self) -> None:
        _require_int(self.ts_event_ns, "ts_event_ns", min_value=0)
        _require_int(self.ts_ingest_ns, "ts_ingest_ns", min_value=0)
        _require_bool(self.frame_complete, "frame_complete")
