# R35C_R5A2_EXECUTION_SHADOW_PNL_FIELDS_LINE_PATCH_NO_REPLAY_NO_ORDER_20260614_002022

classification: REVIEW_R35C_R5A2_PATCH_OR_SAFETY_NEEDS_ATTENTION_NO_REPLAY_NO_ORDER
proof: `run/proofs/R35C_R5A2_EXECUTION_SHADOW_PNL_FIELDS_LINE_PATCH_NO_REPLAY_NO_ORDER_20260614_002022.json`
backup: `run/_code_backups/R35C_R5A2_EXECUTION_SHADOW_PNL_FIELDS_LINE_PATCH_NO_REPLAY_NO_ORDER_20260614_002022_bin_replay_run.py.bak`
...
## Builder context
  ...
  2386	
  2387	
  2388	def _artifact_extract_from_paths(
  2389	    row: Mapping[str, Any],
  2390	    *paths: tuple[str, ...],
  2391	) -> Any:
  2392	    for path in paths:
  2393	        current: Any = row
  2394	        ok = True
  2395	        for key in path:
  2396	            if not isinstance(current, dict) or key not in current:
  2397	                ok = False
  2398	                break
  2399	            current = current[key]
  2400	        if not ok:
  2401	            continue
  2402	
  2403	        value = _artifact_first_present(current)
