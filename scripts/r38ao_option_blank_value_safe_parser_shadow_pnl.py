...
KNOWN_KEYS = {
    "instrument_key", "instrument_role", "ts_event_ns", "provider_id", "provider_role",
    "exchange", "instrument_token", "trading_symbol", "ts_provider_ns", "ts_recv_ns",
    "seq_no", "ltp", "last_qty", "volume", "oi", "bid", "ask", "bid_qty", "ask_qty",
    "bids", "asks", "option_side", "strike", "expiry", "tick_validity", "reject_reason",
    "is_selected_option", "is_shadow_option"
}
...
def stream_id_to_ns(s: str) -> int:
    s = str(s).strip()
    if STREAM_ID_RE.match(s):
        return int(s.split("-", 1)[0]) * 1_000_000
    return 0

def parse_redis_raw_field_value_gz(path: Path):
    """
    Robust Redis raw parser:
    - keeps blank values
    - knows expected field keys
    - flushes only on stream id
    """
    current_id = ""
    fields: dict[str, str] = {}
        ...
        for raw in f:
            ...
            if pending_key is None:
                if line in KNOWN_KEYS:
                    ...
                    # Ignore unexpected dangling value/noise.
                    continue
            else:
...
def parse_option_ticks(opt_gz: Path):
    ...
    for rec in parse_redis_raw_field_value_gz(opt_gz):
        token = str(rec.get("instrument_token") or rec.get("option_token") or "").strip()
        symbol = str(rec.get("trading_symbol") or rec.get("option_symbol") or "").strip()
        role = str(rec.get("instrument_role") or "").strip()
        ltp = safe_float(rec.get("ltp"), 0.0)

        # Join feature generated_at_ns to receiver/stream time, not exchange event time.
        ts = safe_int(rec.get("ts_recv_ns"), 0) or safe_int(rec.get("_stream_ts_ns"), 0)
        event_ts = safe_int(rec.get("ts_event_ns"), 0)

        if token and ltp > 0 and ts > 0:
            ticks[token].append((ts, ltp, symbol, role, event_ts))
            role_counts[role] += 1

    for token in list(ticks):
        ticks[token].sort(key=lambda x: x[0])

    return ticks, role_counts
...
def forward_ltp(ticks, token: str, ts: int, horizon_sec: int):
    ...
    idx = bisect.bisect_left(arr, (target, -1.0, "", "", 0))
    if idx >= len(arr):
        return None
    exit_ts, exit_ltp, symbol, role, event_ts = arr[idx]
    return exit_ltp, exit_ts, symbol, role, event_ts
...
def main() -> int:
    ...
    ticks, role_counts = parse_option_ticks(opt_gz)
    ...
    for r in near:
        ...
        for h in HORIZONS_SEC:
            ...
            if got:
                exit_ltp, exit_ts, exit_symbol, exit_role, exit_event_ts = got
                r[f"ltp_plus_{h}s"] = exit_ltp
                r[f"exit_ts_plus_{h}s"] = exit_ts
                r[f"exit_symbol_plus_{h}s"] = exit_symbol
                r[f"exit_role_plus_{h}s"] = exit_role
                r[f"exit_event_ts_plus_{h}s"] = exit_event_ts
                ...
                r[f"exit_symbol_plus_{h}s"] = None
                r[f"exit_role_plus_{h}s"] = None
                r[f"exit_event_ts_plus_{h}s"] = None
    ...
    summary = {
        "schema": "r38ao_mist_near_option_blank_value_safe_shadow_pnl_summary_v1",
        ...
        "option_role_counts": dict(role_counts),
    ...
    for h in HORIZONS_SEC:
        ...
        for key_field, target in [("branch_id", by_branch), ("failed_stage", by_stage)]:
        ...
        s = stats(vals)
        s["by_branch"] = by_branch
        s["by_stage"] = by_stage
        summary["horizons"][f"{h}s"] = s
    ...
    print("option_role_counts=" + json.dumps(summary["option_role_counts"], sort_keys=True))
